...
exports.createPromotion = async (req, res, next) => {
    try {
        ...
        if (error.code === 11000) {
            return res.status(400).json({ success: false, msg: 'Tên chương trình khuyến mãi đã tồn tại' });
        }
...
exports.getPromotions = async (req, res, next) => {
    try {
        // Populate để lấy thông tin chi tiết của sản phẩm được áp dụng
        const promotions = await Promotion.find({}).populate('appliedProducts', 'name images');
...
exports.updatePromotion = async (req, res, next) => {
    try {
        ...
        if (!promotion) {
            return res.status(404).json({ success: false, msg: `Không tìm thấy khuyến mãi với ID ${req.params.id}` });
        }

...
exports.login = async (req, res, next) => {
    try {
        ...
        if (!email || !password) {
            return res.status(400).json({ success: false, msg: 'Vui lòng cung cấp email và mật khẩu' });
        }
        const user = await User.findOne({ email }).select('+password');
        if (!user) {
            return res.status(401).json({ success: false, msg: 'Email hoặc mật khẩu không chính xác' });
        }
        if (!user.isVerified) {
            return res.status(401).json({ success: false, msg: 'Vui lòng xác thực tài khoản qua email trước khi đăng nhập.' });
        }
        if (user.status === 'vô hiệu hóa') {
            return res.status(403).json({ success: false, msg: 'Tài khoản của bạn đã bị vô hiệu hóa' })
        }
...
exports.verifyEmail = async (req, res, next) => {
    try {
        ...
        if (!user) {
            return res.status(400).json({ success: false, msg: 'Token không hợp lệ hoặc đã hết hạn.' });
        }
        ...
        res.status(200).json({ success: true, msg: 'Xác thực email thành công. Bạn có thể đăng nhập.' });
...
exports.resetPassword = async (req, res, next) => {
    try {
        ...
        if (!password) {
            return res.status(400).json({ success: false, msg: 'Vui lòng cung cấp mật khẩu mới.' });
        }
